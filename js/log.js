async function loginProcess(event) {
      ...
      if (emailField && passwordField) {
        ...
        if (emailValue != "") {
          ...
          if (passwordValue != "") {
            ...
            await getData(emailValue, passwordValue); //llamado del backend usando async y await
  ...
  async function getData(em, pass) {
    try {
      const url = `http://129.151.112.251:8080/api/user/${em}/${pass}`; //REMPLAZAR BASE_URL POR LA URL DEL BACKEND
      const body = {
        email: em,
        password: pass,
      };
